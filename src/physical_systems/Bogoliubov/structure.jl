...
"""
mutable struct Bogoliubov3D{Interp_Scheeme<:abstract_interpolation,Time_Stepping<:abstract_time_stepping}
    name::String #name of the simulation type
    Nk_arguments::Int # Number of arguments of ``n_k``. 1: (fully symetric) , 2: (cylindrical average in 3D or mirror symmetric in 2D), 3: Only mirror symmetric in 3D
    ω # Dispersion relation. This is a function of ``k``. It takes `Nk_argument` arguments
...
@doc raw"""
    Bogoliubov3D(Nk::wave_spectrum; ξ=1.0, c=1.0,, interp_scheeme=lin_interp, time_stepping_scheeme=RK2_step

Constructor of a [`Bogoliubov3D`](@ref) structure. Optionally we set interpolation and time-stepping scheemes:

* `interp_scheeme`: [`lin_interp`](@ref) (default), [`powexp_interp`](@ref), [`powGauss_interp`](@ref), [`BS_interp`](@ref)
* `time_stepping_scheeme`: [`Euler_step`](@ref), [`RK2_step`](@ref) (default), [`ETD2_step`](@ref)

The optional parameter `c=1.0`, `ξ=1.0` are the speed of sound and the healing lenght

"""
function Bogoliubov3D(Nk::wave_spectrum; ξ=1.0, c=1.0, interp_scheeme=lin_interp, time_stepping_scheeme=RK2_step)
    ...
    Bogoliubov3D(name,Nk_arguments,ω,Nk, Stk, F1, FSt, partition,interpScheeme,time_steppingScheeme,diags,t,ξ,c,dimension,dΩ,FD)
end
